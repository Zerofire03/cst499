<?php
    ...
    /**
    * Add an additional org user for login
    */
    function InsertOrgAuthUser($fName, $lName, $username, $password, $orgid)
    {
        global $dbName;
        $conn = getDatabaseConnection($dbName);
        ...
        try
        {
            ...
            // calling stored procedure command
            $sql = 'CALL sp_InsertAuthUser(:_Role, :_VolunteerID, :_OrgID, 
                            :_FirstName, :_LastName, :_UserName, :_Password, 
                            :_LastLogin, :_LastPasswordReset, :_CreatedBy)';
     
            // prepare for execution of the stored procedure
            $stmt = $conn->prepare($sql);
            
            // issue with pass by ref
            $role = 'O';
            ...
            $result = $stmt->fetch();
            $userID = is_array($result) ? $result['lastid'] : 0;
